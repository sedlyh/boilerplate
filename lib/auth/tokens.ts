...
const jwks = createRemoteJWKSet(
    new URL(`https://cognito-idp.${process.env.MY_AWS_REGION}.amazonaws.com/${process.env.COGNITO_USER_POOL_ID}/.well-known/jwks.json`)
);
...
export async function verifyAccessToken(token: string) {
    try {
        ...
        const { payload } = await jwtVerify(token, jwks, {
            issuer: `https://cognito-idp.${process.env.MY_AWS_REGION}.amazonaws.com/${process.env.COGNITO_USER_POOL_ID}`,
        });
